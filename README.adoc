...
The extension tries to mimic behaviour of `[plantuml]` block from https://asciidoctor.org/docs/asciidoctor-diagram[Asciidoctor Diagram] as close as possible.
Positional and named attributes are working as for original exension. Also PNG and SVG formats are supported.
...
Unlike `Asciidoctor Diagram`, the extension uses https://github.com/plantuml/plantuml-server[PlantUML server] for displaying images.
Inside `image` tags produced by it, the `src` attribute contains a link to PlantUML server.

[NOTE]
====
Since `PlantUML server` supports some other formats, it's possible to use http://ditaa.sourceforge.net/[Ditaa syntax] as well.
Check http://plantuml.com/[PlantUML site] for list of supported diagrams.
====
